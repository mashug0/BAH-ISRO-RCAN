...
class Degrador:    
    ...
    def downsample(self , img):
        ...
        downsample = F.interpolate(tensor,
                                   scale_factor=0.5,
                                   mode='bicubic',
                                   align_corners=False,
                                   antialias=True)
        
        return downsample.squeeze(0)
    
    def add_noise(self , img , std_dev = 0.09):
        img = img.unsqueeze(0)
        ...
        def process_and_save(fileinfo):
            try:
                ...
                if random.random() < 0.5:
                    img_ds = self.add_noise(img_ds, std=0.03)
